...
**Ручний QA чекліст (робить юзер):**

*Пошук (keyword):*
- [ ] «Mars» → тільки Mars-нотатка; «mars» / «MARS» → той самий результат (case-insensitive)
- [ ] Частковий збіг: «Mar» → Mars показується (substring по title і content)
- [ ] Збіг тільки в тілі: слово з body знаходить нотатку
- [ ] Промах «xyzzy» → порожній список, жодних чужих нотаток
- [ ] Пробіли навколо запиту — триміться (`" Mars "` = `"Mars"`)
- [ ] Кнопка clear (хрестик) очищає запит → список усіх нотаток
- [ ] Кирилиця: нотатка «Планета Марс», пошук «планет», «МАРС» — знаходить
- [ ] Пошук після редагування: змінив title/content → результат оновлюється без перезапуску
- [ ] Пошук після видалення нотатки зі списку-результатів → рядок зникає

*Флоу нотаток поруч із пошуком:*
- [ ] Створити нотатку, поки існує активний запит → нова з'являється у результатах (якщо збігається)
- [ ] Пошук + повернення з редактора назад → залишається попередній запит і результати
- [ ] Перемикання табів туди-назад — пошук скидається або зберігається (зафіксувати очікувану поведінку)
- [ ] Клавіатура не перекриває список; свайп-догори ховає клавіатуру

*Стани/рестарт/онбординг:*
- [ ] Холодний старт після створення нотаток → вони є, пошук працює
- [ ] Онбординг начисто (skip) → таби рендеряться коректно
- [ ] Empty-state без нотаток: пошук не падає, показує порожньо

*Доступність/локалі:*
- [ ] VoiceOver на кнопках назад і clear — є доречні лейбли («Back», «Clear search»)
- [ ] Dark mode — пошук-бар і результати читабельні
- [ ] Text size Large/Accessibility — не ламає рядки нотаток

*Regression (по-touch семантики):*
- [ ] Сесії медитації завершуються, streak росте (Note + Meditation)
- [ ] AI-draft генерація і колонка інсайтів у Notes працюють як раніше
- [ ] Нотатка навіки в CoreData: рестарт без змін — дані на місці
